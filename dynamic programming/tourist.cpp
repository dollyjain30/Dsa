...
int f (ll i,ll j ,ll x,ll y){
   ...
   ll result = LLONG_MIN;
result = max(result, static_cast<ll>(f(i + 1, j, x + 1, y)));
result = max(result, static_cast<ll>(f(i + 1, j, x, y + 1)));
result = max(result, static_cast<ll>(f(i, j + 1, x + 1, y)));
result = max(result, static_cast<ll>(f(i, j + 1, x, y + 1)));
